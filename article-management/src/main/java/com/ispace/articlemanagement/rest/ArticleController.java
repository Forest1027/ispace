...
public class ArticleController {
    ...
    public ResponseEntity<ArticleDTO> createArticle(@RequestBody ArticleDTO articleDTO, @RequestHeader String idToken) {
        return new ResponseEntity(articleService.createArticle(articleDTO, idToken), HttpStatus.OK);
    }

    @PutMapping("/v1/articles")
    public ResponseEntity<ArticleDTO> updateArticle(@RequestBody ArticleDTO articleDTO, @RequestHeader String idToken) {
        return new ResponseEntity(articleService.updateArticle(articleDTO, idToken), HttpStatus.OK);
    }

    @DeleteMapping("/v1/articles/{id}")
    public ResponseEntity<String> deleteArticleById(@PathVariable int id, @RequestHeader String idToken) {
